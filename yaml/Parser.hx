...
class Parser
{
    ...
    */
	...
	var directiveHandlers:StringMap<String->Array<UnicodeString>->Void>;
	...
	public function safeParseAll(input:UnicodeString, output:Dynamic->Void, options:ParserOptions):Void
	{
		options.schema = new SafeSchema();
		parseAll(input, output, options);
	}

	public function safeParse(input:UnicodeString, options:ParserOptions)
	{
		options.schema = new SafeSchema();
		return parse(input, options);
	}
	
	public function parse(input:UnicodeString, options:ParserOptions):Dynamic
	{
	...
	public function parseAll(input:UnicodeString, output:Dynamic->Void, options:ParserOptions):Void
	{
		...
		directiveHandlers.set('YAML', function(name:String, args:Array<UnicodeString>)
		{
			if (null != version)
		...
		directiveHandlers.set('TAG', function(name:String, args:Array<UnicodeString>)
		{
	...
	// when creating dynamic object graph
	...
	public static var SIMPLE_ESCAPE_SEQUENCES:IntMap<UnicodeString> = 
	{
		var hash = new IntMap<UnicodeString>();
		hash.set(CHAR_DIGIT_ZERO, String.fromCharCode(0x00));// '\x00');
		hash.set(CHAR_SMALL_A, String.fromCharCode(0x07));//'\x07');
		hash.set(CHAR_SMALL_B, String.fromCharCode(0x08));//'\x08');
		hash.set(CHAR_SMALL_T, String.fromCharCode(0x09));//'\x09');
		hash.set(CHAR_TAB, String.fromCharCode(0x09));//'\x09');
		hash.set(CHAR_SMALL_N, String.fromCharCode(0x0A));//'\x0A');
		hash.set(CHAR_SMALL_V, String.fromCharCode(0x0B));//'\x0B');
		hash.set(CHAR_SMALL_F, String.fromCharCode(0x0C));//'\x0C');
		hash.set(CHAR_SMALL_R, String.fromCharCode(0x0D));//'\x0D');
		hash.set(CHAR_SMALL_E, String.fromCharCode(0x1B));//'\x1B');
		hash.set(CHAR_SPACE, String.fromCharCode(0x20));//' ');
		hash.set(CHAR_DOUBLE_QUOTE, String.fromCharCode(0x22));//'\x22');
		hash.set(CHAR_SLASH, String.fromCharCode(0x2f));//'/');
		hash.set(CHAR_BACKSLASH, String.fromCharCode(0x5C));//'\x5C');
		hash.set(CHAR_CAPITAL_N, String.fromCharCode(0x85));//'\x85');
		hash.set(CHAR_UNDERSCORE, String.fromCharCode(0xA0));//'\xA0');
		hash.set(CHAR_CAPITAL_L, String.fromCharCode(0x2028));//'\u2028');
		hash.set(CHAR_CAPITAL_P, String.fromCharCode(0x2029));//'\u2029');
		hash;
	};
